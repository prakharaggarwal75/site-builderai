  ...
  return project ? (
    <div className="flex flex-col h-screen w-full bg-gray-900 text-white">
      ...
      <div className="flex items-center justify-end gap-3 flex-1 text-xs sm:text-sm">
        <button onClick={saveProject} disabled={IsSaving} className="max-sm:hidden bg-gray-800 hover:bg-gray-700 text-white px-3.5 py-1 flex items-center gap-2 rounded m:rounded-sm transition-colors border border-gray-700">
          {IsSaving ? <Loader2Icon className="animate-spin" size={16} /> :
          <SaveIcon size={16} />} Save
        </button>
        <Link target="_blank" to={'/preview/${projectId}'} className="flex items-center gap-2 px-4 py-1 rounded sm:rounded-sm order border-gray-700 hover:border-gray-500 transition-colors">
