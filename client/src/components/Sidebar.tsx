...
useEffect(()=>{
  ...
  return (
    <div className={`${isMenuOpen ? 'block' : 'hidden'} sm:block h-full sm:max-w-sm rounded-xl bg-gray-900 border-gray-800 transition-all ${isMenuOpen ? 'max-sm:w-0 overflow-hidden' : 'w-full'}`}>
        <div className="flex flex-col h-full">
            ...
            <form onSubmit={handleRevisions} className="m-3 relative">
                ...
                <button disabled={IsGenerating || !input.trim()} className="absolute bottom-2.5 right-2.5 rounded-full bg-linear-to-br from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white transition-colors disabled:opacity-60">
                    {IsGenerating
